 ...
 * efficiency.
 * <p>
 * The <code>acks</code> config controls the criteria under which requests are considered complete. The default setting
 * "all" will result in blocking on the full commit of the record, the slowest but most durable setting.
 * <p>
 * If the request fails, the producer can automatically retry. The <code>retries</code> setting defaults to
 * <code>Integer.MAX_VALUE</code>, and it's recommended to use <code>delivery.timeout.ms</code> to control retry
 ...
 * below illustrates how the new APIs are meant to be used. It is similar to the example above, except that all 100
 * messages are part of a single transaction.
 * </p>
 * <p>
 * <pre>
 * {@code
 ...
 * <code>producer.send()</code> or transactional calls hit an irrecoverable error during a transaction. See the
 * {@link #send(ProducerRecord)} documentation for more details about detecting errors from a transactional send.
 * </p>
 * </p>By calling
...
public class KafkaProducer<K, V> implements Producer<K, V> {
     ...
     * {@link Serializer}. Valid configuration strings are documented <a
     * href="http://kafka.apache.org/documentation.html#producerconfigs">here</a>. Values can be either strings or
     * Objects of the appropriate type (for example a numeric configuration would accept either the string "42" or the
     * integer 42).
     * <p>
     * Note: after creating a {@code KafkaProducer} you must always {@link #close()} it to avoid resource leaks.
     *
    ...
    /**
     * Needs to be called before any other methods when the {@code transactional.id} is set in the configuration. This
     ...
     *                                                                    support transactions (i.e. if its version is
     *                                                                    lower than 0.11.0.0)
     ...
     *                                                                    idempotent producer id is unavailable. See the
     *                                                                    exception for more details.  User may retry
     *                                                                    this function call after fixing the
     *                                                                    permission.
     * @throws KafkaException                                             if the producer has encountered a previous
     *                                                                    fatal error or for any other unexpected error
     ...
     * @throws org.apache.kafka.common.errors.InvalidProducerEpochException if the producer has attempted to produce
     ...
     *                                                                      {@code transactional.id} is not authorized.
     *                                                                      See the exception for more details
     * @throws KafkaException                                               if the producer has encountered a previous
     *                                                                      fatal error or for any other unexpected
     *                                                                      error
     */
     ...
     *
     * <p>
     * This method is a blocking call that waits until the request has been received and acknowledged by the consumer
     * group coordinator; but the offsets are not considered as committed until the transaction itself is successfully
     * committed later (via the {@link #commitTransaction()} call).
     *
     * @throws IllegalStateException                                               if no transactional.id has been
     *                                                                             configured, no transaction has been
     *                                                                             started
     * @throws ProducerFencedException                                             fatal error indicating another
     *                                                                             producer with the same
     *                                                                             transactional.id is active
     * @throws org.apache.kafka.common.errors.UnsupportedVersionException          fatal error indicating the broker
     *                                                                             does not support transactions (i.e.
     *                                                                             if its version is lower than
     *                                                                             0.11.0.0)
     ...
     *                                                                             configured transactional.id is not
     *                                                                             authorized, or the consumer group id
     *                                                                             is not authorized.
     * @throws org.apache.kafka.common.errors.InvalidProducerEpochException        if the producer has attempted to
     *                                                                             produce with an old epoch to the
     *                                                                             partition leader. See the exception
     *                                                                             for more details
     * @throws TimeoutException                                                    if the time taken for sending the
     *                                                                             offsets has surpassed
     *                                                                             <code>max.block.ms</code>.
     * @throws KafkaException                                                      if the producer has encountered a
     *                                                                             previous fatal or abortable error, or
     ...
     * <p>
     * This method should be used when you need to batch consumed and produced messages together, typically in a
     * consume-transform-produce pattern. Thus, the specified {@code groupMetadata} should be extracted from the used
     * {@link KafkaConsumer consumer} via {@link KafkaConsumer#groupMetadata()} to leverage consumer group metadata.
     * This will provide stronger fencing than just supplying the {@code consumerGroupId} and passing in
     * {@code new ConsumerGroupMetadata(consumerGroupId)}, however note that the full set of consumer group metadata
     * returned by {@link KafkaConsumer#groupMetadata()} requires the brokers to be on version 2.5 or newer to
     * understand.
     *
     * <p>
     * This method is a blocking call that waits until the request has been received and acknowledged by the consumer
     ...
     *
     * <p>
     * Note, that the consumer should have {@code enable.auto.commit=false} and should also not commit offsets manually
     * (via {@link KafkaConsumer#commitSync(Map) sync} or
     * {@link KafkaConsumer#commitAsync(Map, OffsetCommitCallback) async} commits). This method will raise
     * {@link TimeoutException} if the producer cannot send offsets before expiration of {@code max.block.ms}.
     * Additionally, it will raise {@link InterruptException} if interrupted.
     *
     * @throws IllegalStateException                                               if no transactional.id has been
     *                                                                             configured or no transaction has been
     *                                                                             started.
     * @throws ProducerFencedException                                             fatal error indicating another
     *                                                                             producer with the same
     *                                                                             transactional.id is active
     ...
     *                                                                             does not support transactions (i.e.
     *                                                                             if its version is lower than
     *                                                                             0.11.0.0) or the broker doesn't
     *                                                                             support the latest version of
     *                                                                             transactional API with all consumer
     *                                                                             group metadata (i.e. if its version
     *                                                                             is lower than 2.5.0).
     * @throws org.apache.kafka.common.errors.UnsupportedForMessageFormatException fatal error indicating the message
     *                                                                             format used for the offsets topic on
     *                                                                             the broker does not support
     ...
     *                                                                             retried (e.g. if the consumer has
     *                                                                             been kicked out of the group). Users
     *                                                                             should handle this by aborting the
     *                                                                             transaction.
     * @throws org.apache.kafka.common.errors.FencedInstanceIdException            if this producer instance gets fenced
     *                                                                             by broker due to a mis-configured
     *                                                                             consumer instance id within group
     *                                                                             metadata.
     * @throws org.apache.kafka.common.errors.InvalidProducerEpochException        if the producer has attempted to
     *                                                                             produce with an old epoch to the
     *                                                                             partition leader. See the exception
     ...
     * <p>
     * If the transaction is committed successfully and this method returns without throwing an exception, it is
     * guaranteed that all {@link Callback callbacks} for records in the transaction will have been invoked and
     * completed. Note that exceptions thrown by callbacks are ignored; the producer proceeds to commit the transaction
     * in any case.
     * <p>
     * Note that this method will raise {@link TimeoutException} if the transaction cannot be committed before
     * expiration of {@code max.block.ms}, but this does not mean the request did not actually reach the broker. In
     * fact, it only indicates that we cannot get the acknowledgement response in time, so it's up to the application's
     * logic to decide how to handle timeouts. Additionally, it will raise {@link InterruptException} if interrupted. It
     * is safe to retry in either case, but it is not possible to attempt a different operation (such as
     * abortTransaction) since the commit may already be in the progress of completing. If not retrying, the only option
     * is to close the producer.
     *
     * @throws IllegalStateException                                        if no transactional.id has been configured
     *                                                                      or no transaction has been started
     * @throws ProducerFencedException                                      fatal error indicating another producer with
     *                                                                      the same transactional.id is active
     * @throws org.apache.kafka.common.errors.UnsupportedVersionException   fatal error indicating the broker does not
     *                                                                      support transactions (i.e. if its version is
     *                                                                      lower than 0.11.0.0)
     * @throws org.apache.kafka.common.errors.AuthorizationException        fatal error indicating that the configured
     *                                                                      transactional.id is not authorized. See the
     *                                                                      exception for more details
     * @throws org.apache.kafka.common.errors.InvalidProducerEpochException if the producer has attempted to produce
     *                                                                      with an old epoch to the partition leader.
     *                                                                      See the exception for more details
     * @throws KafkaException                                               if the producer has encountered a previous
     *                                                                      fatal or abortable error, or for any other
     *                                                                      unexpected error
     * @throws TimeoutException                                             if the time taken for committing the
     *                                                                      transaction has surpassed
     *                                                                      <code>max.block.ms</code>.
     * @throws InterruptException                                           if the thread is interrupted while blocked
     */
     ...
     * of {@code max.block.ms}, but this does not mean the request did not actually reach the broker. In fact, it only
     * indicates that we cannot get the acknowledgement response in time, so it's up to the application's logic to
     * decide how to handle timeouts. Additionally, it will raise {@link InterruptException} if interrupted. It is safe
     * to retry in either case, but it is not possible to attempt a different operation (such as
     * {@link #commitTransaction}) since the abort may already be in the progress of completing. If not retrying, the
     * only option is to close the producer.
     *
     * @throws IllegalStateException                                        if no transactional.id has been configured
     *                                                                      or no transaction has been started
     * @throws ProducerFencedException                                      fatal error indicating another producer with
     ...
     *                                                                      fatal error or for any other unexpected
     *                                                                      error
     * @throws TimeoutException                                             if the time taken for aborting the
     *                                                                      transaction has surpassed
     *                                                                      <code>max.block.ms</code>.
     * @throws InterruptException                                           if the thread is interrupted while blocked
     */
...
//    }
     ...
     * was assigned and the timestamp of the record. If the producer is configured with acks = 0, the
     * {@link RecordMetadata} will have offset = -1 because the producer does not wait for the acknowledgement from the
     * broker. If {@link org.apache.kafka.common.record.TimestampType#CREATE_TIME CreateTime} is used by the topic, the
     * timestamp will be the user provided timestamp or the record send time if the user did not specify a timestamp for
     * the record. If {@link org.apache.kafka.common.record.TimestampType#LOG_APPEND_TIME LogAppendTime} is used for the
     * topic, the timestamp will be the Kafka broker local time when the message is appended.
     * <p>
     * Since the send call is asynchronous it returns a {@link java.util.concurrent.Future Future} for the
     * {@link RecordMetadata} that will be assigned to this record. Invoking
     * {@link java.util.concurrent.Future#get() get()} on this future will block until the associated request completes
     * and then return the metadata for the record or throw any exception that occurred while sending the record.
     * <p>
     ...
     * </p>
     * <p>
     * Some transactional send errors cannot be resolved with a call to {@link #abortTransaction()}.  In particular, if
     * a transactional send finishes with a {@link ProducerFencedException}, a
     ...
     * {@link org.apache.kafka.common.errors.AuthorizationException}, then the only option left is to call
     * {@link #close()}. Fatal errors cause the producer to enter a defunct state in which future API calls will
     * continue to raise the same underlying error wrapped in a new {@link KafkaException}.
     * </p>
     * <p>
     * It is a similar picture when idempotence is enabled, but no <code>transactional.id</code> has been configured. In
     * this case, {@link org.apache.kafka.common.errors.UnsupportedVersionException} and
     * {@link org.apache.kafka.common.errors.AuthorizationException} are considered fatal errors. However,
     * {@link ProducerFencedException} does not need to be handled. Additionally, it is possible to continue sending
     * after receiving an {@link org.apache.kafka.common.errors.OutOfOrderSequenceException}, but doing so can result in
     * out of order delivery of pending messages. To ensure proper ordering, you should close the producer and create a
     * new instance.
     * </p>
     * <p>
...
//    }
     ...
     * of
     * <code>flush()</code> is that any previously sent record will have completed (e.g. <code>Future.isDone() ==
     * true</code>). A request is considered completed when it is successfully acknowledged according to the
     * <code>acks</code> configuration you have specified or else it results in an error.
     * <p>
     ...
     * @throws InterruptException      if the thread is interrupted while blocked
     * @throws TimeoutException        if metadata could not be refreshed within {@code max.block.ms}
     * @throws KafkaException          for all Kafka-related exceptions, including the case where this method is called
    ...
    /**
     * Determines the client's unique client instance ID used for telemetry. This ID is unique to this specific client
     * instance and will not change after it is initially generated. The ID is useful for correlating client operations
     * with telemetry sent to the broker and to its eventual monitoring destinations.
     * <p>
     * If telemetry is enabled, this will first require a connection to the cluster to generate the unique client
     * instance ID. This method waits up to {@code timeout} for the producer client to complete the request.
     * <p>
     * Client telemetry is controlled by the {@link ProducerConfig#ENABLE_METRICS_PUSH_CONFIG} configuration option.
     *
     * @param timeout The maximum time to wait for producer client to determine its client instance ID. The value must
     *                be non-negative. Specifying a timeout of zero means do not wait for the initial request to
     *                complete if it hasn't already.
     * @return The client's assigned instance id used for metrics collection.
     * @throws InterruptException       If the thread is interrupted while blocked.
     * @throws KafkaException           If an unexpected error occurs while trying to determine the client instance ID,
     *                                  though this error does not necessarily imply the producer client is otherwise
     *                                  unusable.
     * @throws IllegalArgumentException If the {@code timeout} is negative.
     * @throws IllegalStateException    If telemetry is not enabled ie, config `{@code enable.metrics.push}` is set to
     *                                  `{@code false}`.
     */
    @Override
    public Uuid clientInstanceId(Duration timeout) {
        if (!clientTelemetryReporter.isPresent()) {
            throw new IllegalStateException(
                    "Telemetry is not enabled. Set config `" + ProducerConfig.ENABLE_METRICS_PUSH_CONFIG
                            + "` to `true`.");
        }
    ...
    /**
     * Close this producer. This method blocks until all previously sent requests complete. This method is equivalent
     * to
     ...
     *
     * @throws InterruptException If the thread is interrupted while blocked.
     * @throws KafkaException     If an unexpected error occurs while trying to close the client, this error should be
     *                            treated as fatal and indicate the client is no longer usable.
     */
    @Override
    public void close() {
    ...
    /**
     * This method waits up to <code>timeout</code> for the producer to complete the sending of all incomplete
     * requests.
     * <p>
     * If the producer is unable to complete all requests before the timeout expires, this method will fail any unsent
     * and unacknowledged records immediately. It will also abort the ongoing transaction if it's not already
     * completing.
     * <p>
     ...
     *                non-negative. Specifying a timeout of zero means do not wait for pending send requests to
     *                complete.
     * @throws InterruptException       If the thread is interrupted while blocked.
     * @throws KafkaException           If an unexpected error occurs while trying to close the client, this error
     *                                  should be treated as fatal and indicate the client is no longer usable.
    ...
    private void close(Duration timeout, boolean swallowException) {
        ...
        clientTelemetryReporter.ifPresent(
                reporter -> Utils.closeQuietly(reporter, "producer telemetry reporter", firstException));
    ...
    /**
     * computes partition for given record. if the record has partition returns the value otherwise if custom
     * partitioner is specified, call it to compute partition otherwise try to calculate partition based on key. If
     * there is no key or key should be ignored return RecordMetadata.UNKNOWN_PARTITION to indicate any partition can be
     * used (the partition is then calculated by built-in partitioning logic).
     */
    ...
    private class AppendCallbacks implements RecordAccumulator.AppendCallbacks {
        ...
        private AppendCallbacks(Callback userCallback, ProducerInterceptors<K, V> interceptors,
        ...
        public void setPartition(int partition) {
            ...
            if (log.isTraceEnabled()) {
                ...
                log.trace("Attempting to append record {} with callback {} to topic {} partition {}", recordLogString,
                        userCallback, topic, partition);
            }
        }
